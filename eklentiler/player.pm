...
async def yplay(_, message: Message):
    ...
    if type=="audio":
        if round(m_audio.audio.duration / 60) > DURATION_LIMIT:
            d=await message.reply_text(f"❌ Daha uzun audiolar {DURATION_LIMIT} minute(s) icazə verilmir, təqdim olunan audiodur {round(m_audio.audio.duration/60)} minute(s)")
            await mp.delete(d)
            await mp.delete(message)
            return
        if playlist and playlist[-1][2] \
                == m_audio.audio.file_id:
            d=await message.reply_text(f"{emoji.ROBOT} Artıq Pleylistə əlavə edilib")
            await mp.delete(d)
            await mp.delete(message)
            return
        now = datetime.now()
        ...
        if len(playlist) == 1:
            m_status = await message.reply_text(
                f"{emoji.INBOX_TRAY} Yükləmə və Emal..."
            )
    ...
    if type=="youtube" or type=="query":
        if type=="youtube":
            msg = await message.reply_text("⚡️ **YouTube-dan Mahnı Götürülür...**")
            url=yturl
        elif type=="query":
            try:
                msg = await message.reply_text("⚡️ **YouTube-dan Mahnı Götürülür...**")
...
async def deezer(_, message):
    ...
    group_call = mp.group_call
    if album:
        msg = await message.reply("⚡️ **JioSaavn-dan Albom alınır...**")
        try:
        ...
        msg = await message.reply("⚡️ **JioSaavn-dan Mahnı Alınır...**")
        try:
        ...
        try:
            ...
            for i in items:
                if 2 <= i <= (len(playlist) - 1):
                    ...
                    text.append(f"{emoji.CROSS_MARK} İlk iki mahnını keçə bilməzsiniz- {i}")
...
async def join_group_call(client, m: Message):
    ...
    if group_call.is_connected:
        k=await m.reply_text(f"{emoji.ROBOT} Artıq səsli çata qoşulub")
        await mp.delete(k)
        await mp.delete(m)
        return
    await mp.start_call()
    chat = await client.get_chat(CHAT)
    k=await m.reply_text(f"Səsli Çata Uğurla Qoşuldu {chat.title}")
    await mp.delete(k)
    await mp.delete(m)
...
async def leave_voice_chat(_, m: Message):
    group_call = mp.group_call
    if not group_call.is_connected:
        k=await m.reply_text("Hələ heç bir Voicechat-a qoşulmayıb.")
        await mp.delete(k)
        await mp.delete(m)
        return
    playlist.clear()
    ...
    k=await m.reply_text("Səsli Çatı tərk etdi")
    await mp.delete(k)
    await mp.delete(m)
...
async def list_voice_chat(client, m: Message):
    ...
    if group_call.is_connected:
        ...
        k=await m.reply_text(emoji.NO_ENTRY
                                   + "Hələ heç bir səsli söhbətə qoşulmadınız")
    await mp.delete(k)
    await mp.delete(m)
...
async def stop_playing(_, m: Message):
    group_call = mp.group_call
    if not group_call.is_connected:
        k=await m.reply_text("Dayanacaq heç nə oynamır.")
        await mp.delete(k)
        await mp.delete(m)
        return
    if 1 in RADIO:
...
async def not_chat(_, m: Message):
    buttons = [
        ...
        [
            InlineKeyboardButton('👨‍💻 inkişaf etdirici', url='https://t.me/A_l_i_y_e_v_d_i'),
            InlineKeyboardButton('kanal 📢', url='https://t.me/iron_Blood_Gurup')
        ]
        ]
    k=await m.reply("<b>🇦🇿🇦🇿🇦🇿 [🇦🇿🇦🇿🇦🇿](..) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
    await mp.delete(m)
